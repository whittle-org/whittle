...
class CausalSelfAttention(nn.Module):
    ...
    def set_sub_network(
        ...
        if self.config.n_query_groups == 1:
            ...
            self.sub_network_query_groups = self.sub_network_n_head // (self.config.n_head // self.config.n_query_groups)
        if self.config.fix_head_size:
           self.sub_network_head_size = self.config.head_size
           self.sub_network_qkv_shape = (self.sub_network_n_head+ 2 * self.sub_network_query_groups) * self.sub_network_head_size
        else:
           self.sub_network_head_size = self.sub_network_n_embd // self.sub_network_n_head
    ...
    def forward(
        ...
        assert self.sub_network_n_embd is not None, "You need to call `gpt.set_sub_network()"
        ...
        if self.config.fix_head_size:
            qkv = qkv.view(B, T, self.sub_network_query_groups, total_qkv, self.sub_network_head_size)
        else:
        ...
        if self.config.fix_head_size:
          if self.sub_network_query_groups != self.sub_network_n_head and (input_pos is None or self.config.n_query_groups != 1):          
            k = k.expand(B, self.sub_network_query_groups, q_per_kv, T, self.sub_network_head_size)
            v = v.expand(B, self.sub_network_query_groups, q_per_kv, T, self.sub_network_head_size)
        else:
        ...
        if self.config.fix_head_size:
            ...
            v = torch.nn.functional.pad(v,(0,abs(self.config.head_size-self.sub_network_head_size)))
            #print(q.shape)
            #print(k.shape)
            #print(v.shape)
        if self.config.fix_head_size:
        ...
        #print(y.shape)
        #print(self.sub_network_n_head)
        #print(self.sub_network_haed_size)
        print(y.shape)
        y = y.reshape(B, T, self.sub_network_head_size * self.sub_network_n_head)  # re-assemble all head outputs side by side
        print("Y",y.shape)
        # output projection
        return self.proj(y)
