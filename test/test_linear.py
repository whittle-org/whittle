...
def test_linear_equivalence_sample_in_indices():
    ...
    linear.weight.data = (
        torch.arange(supernet_out_features)[:, None]
        + torch.arange(supernet_in_features) * 3.14
    )
    ...
    new_linear.weight.data = (
        torch.arange(supernet_out_features)[:, None]
        + torch.tensor(sampled_in_indices) * 3.14
    )
...
def test_linear_equivalence_sample_out_indices():
    ...
    new_linear.weight.data = (
        torch.tensor(sampled_out_indices)[:, None]
        + torch.arange(supernet_in_features) * 3.14
    )
