...
def test_merge_extract(checkpoint_dir):
    ...
    random_input = torch.randint(0, 8, size=(2, 4), dtype=torch.int64)
    pre_merge_y = model(random_input)
    ...
    for pre_module, post_module in zip(pre_modules, post_modules):
        if isinstance(pre_module, (LoRALinear, LoRALinearProj, LoRAQKVLinear)):
            input_data = torch.randn(32, pre_module.in_features)
        elif isinstance(pre_module, LoRAEmbedding):
            input_data = torch.randint(0, 8, size=(2, 4), dtype=torch.int64)
        else:
            continue
