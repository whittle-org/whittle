...
from slm_baselines.utils_eval import (
    plot_validation_metrics,
    plot_accuracies,
    evaluate_slm,
    evaluate_slm_lambada,
)
...
def main(
    ...
    load_checkpoint(
        fabric,
        model,
        model_path + "/finetune/lora/final/lit_model.pth.lora",
        strict=False,
    )
...
def fit(
    ...
    name = f"{model_path}/eval_{downstream_dataset}.pkl"
    if not os.path.exists(name):
        if downstream_dataset == "lambada":
            acc = evaluate_slm_lambada(
                name,
                model,
                subnet_config,
                downstream_dataset,
    ...
    # with open(f"grid_eval_{downstream_dataset}_calibrate_mag_llama-2-7b.pkl","wb") as f:
    # pickle.dump({"accs":accs, "params":params},f)
