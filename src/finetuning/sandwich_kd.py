...
class SandwichStrategy(BaseTrainingStrategy):
    ...
    def __init__(self, random_samples=3, weight_supernet_loss=False, **kwargs):
        ...
        self.lora = False
        self.loss_function_base = chunked_cross_entropy
    ...
    def __call__(self, model, inputs, outputs, step, kd_loss, **kwargs):
        ...
        if self.lora:
            ...
            # y_supernet[-1] = y_supernet[-1][..., :-1, :]
        # loss = self.loss_function.compute_loss(y_supernet.reshape(-1, vocab), y_supernet.reshape(-1, vocab), outputs[..., 1:].reshape(-1), loss_type=kd_loss)
        if self.weight_supernet_loss:
            loss = (self.random_samples) * self.loss_function_base(
                y_supernet, outputs[..., 1:]
            )
        else:
            loss = self.loss_function_base(y_supernet, outputs[..., 1:])
        ...
        for i in range(self.random_samples):
            ...
            loss = self.loss_function.compute_loss(
                y_hat.reshape(-1, vocab),
                y_supernet.reshape(-1, vocab).detach(),
                outputs[..., 1:].reshape(-1),
                loss_type=kd_loss,
            )
            self.fabric.backward(loss / (step * loss_factor))
