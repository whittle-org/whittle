...
def setup(
    ...
    n_trials: int = 10000,
    downstream_test_iters: int = 500000000,
    ...
    if train_strategy == "sandwich-kd":
        out_dir = Path(
            f"{config.name}-{train_strategy}-{search_space_type}-{sampling_strategy}-{kd_loss}-{time_string}/finetune/galore/"
        )
        id = f"{train_strategy}-{search_space_type}-{sampling_strategy}-{kd_loss}-{time_string}-galore"
    else:
        out_dir = Path(
            f"{config.name}-{train_strategy}-{search_space_type}-{sampling_strategy}/finetune/galore/"
        )
        id = f"{train_strategy}-{search_space_type}-{sampling_strategy}-{time_string}-galore"
...
def main(
    ...
    optimizer = GaLoreAdamW(param_groups, lr=1e-5, weight_decay=0)
