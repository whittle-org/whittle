...
def setup(
    ...
    x_metric_df = x_metric_df.set_index('sub_network').drop(columns=['task', 'metric']).rename(columns={'score': metric})
    ...
    for task in tasks:
        ...
        drop_cols = ['task', 'metric', 'pareto'] if 'pareto' in task_df.columns else ['task', 'metric']
        task_df = task_df.set_index('sub_network').drop(columns=drop_cols).rename(columns={'score': y_name})

        results = x_metric_df.join(task_df, how='inner')
        print(f"Plotting {len(results)} sub-networks, task: {task}, metric: {metric}")

        plt.figure()
        sns.scatterplot(data=results, x=metric, y=y_name, hue='pareto' if 'pareto' in results.columns else None)
